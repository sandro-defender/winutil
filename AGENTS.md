...
1. **Do not edit files under `compiled/` directly.** They are generated build output (see SPEC.md's Build Model). Change source files and compile.
2. **Do not commit files under `compiled/`.** They are ignored locally and generated by GitHub Actions for releases.
...
- Compile:
  ```powershell
  .\compiler\Compile.ps1
  ```
- Compile and run GUI:
  ```powershell
  .\compiler\Compile.ps1 -Run
  ```
...
For changes that affect the compiled WinUtil script, make them only in the source files described in SPEC.md's Repository Layout — never in `compiled/winutil.ps1` itself. If behavior changes require the compiled script to change, update the source files and run `.\compiler\Compile.ps1` only to verify generation.
...
- For compile/build changes, run `.\compiler\Compile.ps1`.
- For GUI behavior changes, run `.\compiler\Compile.ps1 -Run` when practical and verify the affected path manually.
...
- Treat local `compiled/` changes as disposable compile output.
- Never stage or commit `compiled/`, `binary/`, or anything else ignored by the root `.gitignore` or `docs/.gitignore` — read those files rather than assuming. `docs/public/` is tracked source for static assets, not generated output.
